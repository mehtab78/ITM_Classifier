...
#
# This program has been adapted and rewritten from the CMP9137 materials of 2024.
#
# It treats the task of multi-choice visual question answering as a binary
# classification task. The original multi-choice problem is reformulated as a
# binary classification where each candidate answer (paired with the question)
# is labeled "match" or "no-match".
#
# The implementation includes:
# - ITM_Dataset: Loads image data and corresponding text embeddings,
#   raw question and answer text, and binary labels.
# - ITM_Model: A model that fuses visual features from a CNN (or ViT) with text
#   features (from two separate linear transformations of the question and answer
#   embeddings) and outputs logits.
...
class ITM_Dataset(Dataset):
    ...
    def load_data(self):
        ...
        with open(self.data_file, "r") as f:
            ...
            for line in lines:
                ...
                # Split by '?' to separate question and answer portions
    ...
    def __getitem__(self, idx):
        ...
        question = (
            self.question_data[idx]
            if hasattr(self, "question_data")
            else "Unknown question"
        )
        answer = (
            self.answer_data[idx] if hasattr(self, "answer_data") else "Unknown answer"
        )
        return (
            img,
            question_embedding,
...
# Pre-trained ViT model (if using ViT)
class Transformer_VisionEncoder(nn.Module):
    def __init__(self, pretrained=None):
        ...
        if pretrained:
            ...
            # Unfreeze last two layers of heads
...
class ITM_Model(nn.Module):
    def __init__(self, num_classes=2, ARCHITECTURE=None, PRETRAINED=None):
        ...
        if self.ARCHITECTURE == "CNN":
            ...
            exit(0)
        # Two separate text embedding processing layers
...
def evaluate_model(model, ARCHITECTURE, test_loader, criterion, device):
    ...
    # Calculate extra metrics (balanced accuracy, etc.) if needed
